...
#define CURRENT_SCALE   0.00402832f
#define GEAR_RATIO  8.0f  // 减速比 8:1
#define VOLTAGE_LIMIT   12.0f //该宏定义最好改成变量，根据实际电压和测量范围调整，确保 PID 输出不会过饱和和达不到
...
// --- 变量 ---
int   calib_cnt = 0;
float offset_Ia = 0.0f, offset_Ic = 0.0f;
...
uint16_t Ic_Raw = 0; 
float    Ib_Calc = 0.0f; // 由 Ia/Ic 计算得到，便于观察
uint16_t Vbus_Raw = 0;
float    Vbus = 0.0f; // 由 Ia/Ic 计算得到，便于观察
...
// --- 电流环参数 (根据带宽计算) ---
// 电流环带宽建议设为采样频率的1/10~1/20，即 1kHz~2kHz
float current_bandwidth = 1000.0f;    // 电流环带宽 (Hz)
// 计算电流环 PI 参数
// Ti = L/R, Kp = L * wc (wc = 2π * fc)
...
float current_Ki = singlephase_R * current_wc;           // Ki = R * wc = Kp/Ti
...
float Speed_Ramp_Target = 0.0f; // 经过斜坡处理后的目标速度
float Ramp_Step = 0.2f;         // 斜坡步长，决定加减速快慢 (值越小越柔和)
...
void main(void)
{
    ...
    // --- PID 参数初始化 (根据带宽计算) ---
    // 1. 电流环 (20kHz) - 根据带宽计算
    // 计算公式: Ti = L/R, Kp = L*wc, Ki = R*wc (wc = 2π*fc)
    pid_id.Kp = current_Kp;   pid_id.Ki = current_Ki;
    pid_id.OutMax = VOLTAGE_LIMIT; pid_id.OutMin = -VOLTAGE_LIMIT;
    pid_iq.Kp = current_Kp;   pid_iq.Ki = current_Ki;
    pid_iq.OutMax = VOLTAGE_LIMIT; pid_iq.OutMin = -VOLTAGE_LIMIT;
    // 2. 速度环 (2kHz)
...
__interrupt void INT_IA_IC_ADCA_INT1_ISR(void) 
{
    ...
    Vbus_Raw = ADC_readResult(ADCCRESULT_BASE, ADC_SOC_NUMBER1);
    Vbus = 0.01933594f * Vbus_Raw;
    Ic_Raw = ADC_readResult(ADCARESULT_BASE, ADC_SOC_NUMBER1);
    foc.DC_Bus_V = Vbus;
    switch(Sys_State)
    {
        // --- 状态0: 电流零点校准 ---
        ...
        case STATE_CLOSED_LOOP:
            ...
            foc.Ic = ((float)Ic_Raw - offset_Ic) * CURRENT_SCALE; // Ic 方向需与 Ia 一致，如相反请在此加负号
            Ib_Calc = -(foc.Ia + foc.Ic);
            foc.Ib = Ib_Calc; // Ia + Ib + Ic = 0
            ...
            FOC_Park(&foc);
             ...
             // 简单的滤波用于显示
            Id_disp = 0.7f * Id_disp + 0.3f * foc.Id;
            Iq_disp = 0.7f * Iq_disp + 0.3f * foc.Iq;

            // [三环控制逻辑]
            loop_cnt++;if(loop_cnt>30)loop_cnt=0;

            float Vq_Target = 0.0f;
            float raw_target_speed = 0.0f; // 暂存原始的目标速度
            ...
            switch(Control_Mode)
            {
                ...
                case CTRL_MODE_SPEED:
                    ...
                    if(loop_cnt >= 10) 
                    {
                        ...
                        // 速度给定使用斜坡，防止突变
                        raw_target_speed = User_Speed_Ref * GEAR_RATIO;
                        float delta_speed = raw_target_speed - Speed_Ramp_Target;
                        if(delta_speed > Ramp_Step) delta_speed = Ramp_Step;
                        else if(delta_speed < -Ramp_Step) delta_speed = -Ramp_Step;
                        Speed_Ramp_Target += delta_speed;
                        pid_spd.Ref = Speed_Ramp_Target; 
                        pid_spd.Fbk = foc.Speed_RPM;
                        PID_Calc(&pid_spd);         
                        foc.Iq_Ref = pid_spd.Out;
                    }
